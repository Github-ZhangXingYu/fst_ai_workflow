...
def configure_and_build(build_dir: str, target: str = None,
    ...
    if coverage:
        ...
        cmake_args.append('-DCMAKE_BUILD_TYPE=Coverage')
    ...
    try:
        config_result = subprocess.run(
            ...
            env={**os.environ, 'CMAKE_BUILD_PARALLEL_LEVEL': str(parallel_jobs)}
        )
...
def main():
    parser = argparse.ArgumentParser(
        description='FST编译运行器：CMake构建、错误解析、clang-tidy分析'
    )
    parser.add_argument('--build-dir', default='build/test',
                        help='构建目录（默认build/test）')
