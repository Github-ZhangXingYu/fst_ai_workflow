...
对于每个 `verdict` 为 `adapt` 或 `new` 的函数，需要你（AI）进一步分析：
- 阅读函数源码，理解其逻辑
- 审视已有的测试，判断测试质量
- 确认或修正评估结论
...
1. **阅读函数源码**：用 Read 工具获取函数完整代码
2. **分析测试需求**：
   - 识别所有逻辑分支（if/else、循环、switch、异常路径）
   - 识别外部依赖（需要 Mock 的类和接口）
   - 识别边界条件（空输入、极限值、nullptr）
3. **生成测试代码**：
   ...
   - **性能测试** → `test/<模块>/performance/bench_<函数名>.cpp`
     - 使用 Google Benchmark
     - 设置合理的数据规模
4. **参考模板** 在 `config/templates/` 目录
5. **验证**：生成后检查代码基本正确性（语法、include、namespace）
6. 写入测试文件，记录审计日志
